...
function getStatusText(item) {
    // SCD, SMCD, SM, GS 등 상태코드 필드 확인
    var statusCode = item.SCD || item.SMCD || item.SM || item.GS || item.STAT || item.STATUS || '';
    if (statusCode && STATUS_CODE_MAP[statusCode.trim()]) {
        return STATUS_CODE_MAP[statusCode.trim()];
    }

    // 한글 상태 직접 제공 필드 확인
    var statusName = item.SMION || item.SMI || item.statusName || item.SMNM || '';
    if (statusName && statusName !== '&nbsp;' && statusName.trim()) {
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 첫 번째 항목 전체 필드 로깅 (디버깅 - 상태 필드 확인)
        if (resultList.length > 0) {
            const firstItem = resultList[0];
            console.log('📋 첫 번째 항목 전체 필드:', JSON.stringify(firstItem, null, 2));
            console.log('📋 전체 필드 키 목록:', Object.keys(firstItem).join(', '));
            // 상태 관련 필드 후보 로깅
            console.log('📋 상태 관련 필드: SCD=' + firstItem.SCD + ', SMCD=' + firstItem.SMCD + ', SM=' + firstItem.SM + ', GS=' + firstItem.GS + ', STAT=' + firstItem.STAT + ', STATUS=' + firstItem.STATUS + ', SMION=' + firstItem.SMION + ', SMI=' + firstItem.SMI + ', SMNM=' + firstItem.SMNM);
        }
