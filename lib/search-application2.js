...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 각 특허의 현재상태에 따라 KIPRIS API로 전문보기 URL 조회
        console.log('📄 전문보기 URL 조회 시작...');
        await Promise.all(patents.map(async function(patent) {
            try {
                ...
                if (patent.currentStatus === '등록') {
                    var result = await patentService.getAnnouncementFullTextUrl(appNo);
                    patent.fullTextUrl = (result && result.path) ? result.path : '-';
                } else if (patent.currentStatus === '공개') {
                    var result = await patentService.getPublicationFullTextUrl(appNo);
                    patent.fullTextUrl = (result && result.path) ? result.path : '-';
                } else {
                    patent.fullTextUrl = '-';
                }
            } catch (err) {
                console.error('전문보기 URL 조회 실패:', patent.applicationNumber, err.message);
                patent.fullTextUrl = '-';
            }
        }));
        console.log('✅ 전문보기 URL 조회 완료');
