...
// 일시적 네트워크 오류 시 재시도 (키프리스 서버가 간헐적으로 연결을 끊는 ECONNRESET 등 대응)
const RETRYABLE_CODES = ['ECONNRESET', 'ETIMEDOUT', 'ECONNABORTED', 'ECONNREFUSED', 'EPIPE'];

async function postWithRetry(url, body, options, maxRetries = 2) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`⚠️ 키프리스 요청 실패 (${error.code}), ${delayMs}ms 후 재시도 ${attempt + 1}/${maxRetries}...`);
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const response = await postWithRetry('https://www.kipris.or.kr/kpat/resulta.do', formData.toString(), {
            headers: {
        ...
        const patents = resultList.map(function(item) {
            ...
            return {
                ...
                inventionTitleEng: cleanValue(item.TLEE),
        ...
        // 전문보기 유형을 상태값으로만 결정 (KIPRIS API 사전 확인 없음 - 클릭 시에만 API 사용)
        patents.forEach(function(patent) {
            if (patent.currentStatus === '등록') {
                patent.fullTextType = 'ann';
            } else if (patent.currentStatus === '공개') {
                patent.fullTextType = 'pub';
            } else {
                patent.fullTextType = '-';
            }
        });
